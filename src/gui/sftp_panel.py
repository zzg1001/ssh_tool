...
class SFTPPanel:
    ...
    def _setup_ui(self):
        ...
        # 工具栏
        toolbar = tk.Frame(self.frame, bg='#f8f8f8', pady=3)
        ...
        # 分隔线
        tk.Frame(toolbar, width=1, bg='#ddd').pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=2)
        ...
        # 路径栏
        path_frame = tk.Frame(self.frame, bg='#ffffff', pady=5)
        ...
        self.path_entry.pack(fill=tk.X, padx=5)
        self.path_entry.bind('<Return>', self._on_path_enter)

        # 文件列表 - 使用 Treeview 支持图标
        ...
        self.file_tree = ttk.Treeview(
            ...
            show='tree',  # 只显示树形结构，不显示表头
            selectmode='extended'
        )
        self.file_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        scrollbar = tk.Scrollbar(list_frame, command=self.file_tree.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.file_tree.configure(yscrollcommand=scrollbar.set)

        # 绑定事件
        self.file_tree.bind('<Double-1>', self._on_double_click)
        self.file_tree.bind('<Button-3>', self._on_right_click)  # 右键菜单

        # 拖放支持 - 绑定拖放事件
        self._setup_drag_drop()

        # 保存文件信息 (item_id -> file_info)
        self._files = {}

        # 进度条区域
        progress_frame = tk.Frame(self.frame, bg='#ffffff', pady=5)
        ...
        # 底部选项
        options_frame = tk.Frame(self.frame, bg='#ffffff', pady=5)
        options_frame.pack(fill=tk.X, side=tk.BOTTOM)

        self.follow_check = tk.Checkbutton(
            options_frame,
            text="Follow terminal folder",
    ...
    def _parse_drop_data(self, data):
        ...
        files = []
        if data.startswith('{'):
            # macOS 格式: {/path/to/file1} {/path/to/file2}
            import re
    ...
    def _upload_files(self, local_files):
        ...
        def upload():
            ...
            for i, local_path in enumerate(local_files):
                ...
                self.frame.after(0, lambda f=filename: self.progress_label.config(
                    text=f"上传: {f}"
                ))
    ...
    def _on_mkdir(self):
        ...
        tk.Label(dialog, text="文件夹名称:").pack(pady=(15, 5))
        ...
        tk.Button(dialog, text="创建", command=create).pack(pady=10)
